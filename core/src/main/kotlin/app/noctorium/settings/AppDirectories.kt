...
/**
 * The one folder Noctorium keeps everything it owns in.
 *
 * The application has been called Spice, Spicetify and Spiceity before this, and that folder holds a great
 * deal that cannot simply be made again: the encrypted credentials, both cookie jars, the downloaded yt-dlp
 * and mpv binaries and the bundled browser, together several hundred megabytes. Renaming the application
 * must not orphan any of it, so the first run under the new name moves the old folder across. Within one volume that is a rename
...
object AppDirectories {
    ...
    internal const val CURRENT = "Noctorium"

    /**
     * Every name the folder has gone by, newest first.
     *
     * The application has been renamed three times, and someone may be arriving from any of them. Keeping
     * the whole chain means an install that skipped a version is picked up just as well as one that did
     * not, and the folder is only ever adopted once because the check is whether the current name already
     * exists.
     */
    internal val PREVIOUS = listOf("Spiceity", "Spicetify", "Spice")
