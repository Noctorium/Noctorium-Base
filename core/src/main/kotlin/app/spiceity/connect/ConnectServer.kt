...
class ConnectServer(
    ...
    /** Ephemeral, and told to the network in the announcement, so two Spiceitys on one machine both work. */
    ...
    private suspend fun serve(client: Socket) {
        client.use { socket ->
            ...
            val secret = key()
            if (secret == null) {
                respond(output, 503, """{"ok":false,"message":"This device is not signed in to Spiceity."}""")
                return
            }
    ...
    companion object {
        const val HEADER_NONCE = "x-spiceity-nonce"
        const val HEADER_TIMESTAMP = "x-spiceity-timestamp"
        const val HEADER_AUTH = "x-spiceity-auth"
        const val HEADER_DEVICE = "x-spiceity-device"
