 ...
 *
 * ```
 * NOCTORIUM_LIVE_CHECK=1 ./gradlew test --tests 'app.noctorium.account.LiveServiceCheck'
 * ```
 *
 * The accounts it makes end `@noctorium.invalid`, a domain that can never receive mail, so they are safe
...
class LiveServiceCheck {
    ...
    fun `the app's own client can sign up, report listens and read them back`() = runBlocking {
        ...
        val client = NoctoriumAccountClient()
        val email = "applink-${randomText(4)}@noctorium.invalid"
    ...
    fun `an unreachable service is reported as unreachable, not as a refusal`() = runBlocking {
        ...
        val client = NoctoriumAccountClient(baseUrl = "http://127.0.0.1:1")
