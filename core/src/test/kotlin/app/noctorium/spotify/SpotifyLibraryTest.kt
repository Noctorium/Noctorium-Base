...
class SpotifyAuthTest {
     ...
     * could not have been. It is also the one address in Noctorium that is deliberately not HTTPS: it never
     * leaves the machine.
     */
    @Test
    fun `the redirect address is the loopback one that gets registered`() {
        assertEquals("http://127.0.0.1:8888/noctorium/spotify", SpotifyAuth.redirectUri())
    }
    ...
    fun `the consent address proves the request without carrying a secret`() {
        ...
        assertTrue(url.contains("redirect_uri=http%3A%2F%2F127.0.0.1%3A8888%2Fnoctorium%2Fspotify"))
    ...
    fun `only Spotify refusing the sign-in counts as a refusal`() {
        ...
        // A request Noctorium got wrong is a bug here, not a revoked sign-in.
