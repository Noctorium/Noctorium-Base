...
class SettingsRepositoryTest {
    ...
    fun `account and profile preferences survive reload`() {
        ...
        try {
            ...
            val expected = NoctoriumPreferences(
                profileName = "Yabosen",
    ...
    fun `settings written by older builds keep their browser choice`() {
        val directory = Files.createTempDirectory("noctorium-settings-migration-test")
        try {
    ...
    fun `missing settings file uses safe defaults`() {
        val directory = Files.createTempDirectory("noctorium-settings-default-test")
        try {
            assertEquals(NoctoriumPreferences(), SettingsRepository(directory.resolve("missing.json")).load())
        } finally {
            directory.toFile().deleteRecursively()
        }
    }
...
class DiscordMigrationTest {
    ...
    fun `an older build's discord switch carries over to the new settings`() {
        val directory = Files.createTempDirectory("noctorium-discord-migration")
        try {
            val path = directory.resolve("settings.json")
    ...
    fun `a build with no discord settings at all starts disabled`() {
        val directory = Files.createTempDirectory("noctorium-discord-default")
        try {
            val path = directory.resolve("settings.json")
