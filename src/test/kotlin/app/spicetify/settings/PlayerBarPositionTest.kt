...
class PlayerBarPositionTest {
    ...
    fun `the bar stays at the foot of the window unless asked otherwise`() {
        assertEquals(PlayerBarPosition.BOTTOM, SpicetifyPreferences().playerBarPosition)
    }

    @Test
    fun `settings saved before the choice existed still open at the bottom`() {
        val older = """{"profileName":"Spicetify Listener","playerBarStyle":"INLINE"}"""

        val preferences = json.decodeFromString<SpicetifyPreferences>(older)
    ...
    fun `the choice survives being written and read back`() {
        val chosen = SpicetifyPreferences(playerBarPosition = PlayerBarPosition.TOP)

        val restored = json.decodeFromString<SpicetifyPreferences>(json.encodeToString(chosen))
    ...
    fun `position and layout do not disturb each other`() {
        val stackedOnTop = SpicetifyPreferences(
            playerBarStyle = PlayerBarStyle.STACKED,
            playerBarPosition = PlayerBarPosition.TOP,
        )

        val restored = json.decodeFromString<SpicetifyPreferences>(json.encodeToString(stackedOnTop))
