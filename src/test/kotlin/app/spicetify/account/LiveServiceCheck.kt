 ...
 *
 * ```
 * SPICETIFY_LIVE_CHECK=1 ./gradlew test --tests 'app.spicetify.account.LiveServiceCheck'
 * ```
 *
 * The accounts it makes end `@spicetify.invalid`, a domain that can never receive mail, so they are safe
 * to delete at any time.
 */
class LiveServiceCheck {
    private val asked = System.getenv("SPICETIFY_LIVE_CHECK")?.trim() == "1"
    ...
    fun `the app's own client can sign up, report listens and read them back`() = runBlocking {
        if (!asked) {
            println("LIVE: skipped; set SPICETIFY_LIVE_CHECK=1 to run it")
            return@runBlocking
        }
        val client = SpicetifyAccountClient()
        val email = "applink-${randomText(4)}@spicetify.invalid"
    ...
    fun `an unreachable service is reported as unreachable, not as a refusal`() = runBlocking {
        ...
        val client = SpicetifyAccountClient(baseUrl = "http://127.0.0.1:1")
