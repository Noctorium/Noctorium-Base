...
import app.spicetify.settings.BrowserSession
import app.spicetify.settings.CookieSource
import app.spicetify.settings.SpicetifyPreferences
...
class LibraryEligibilityTest {
    ...
    fun `soundcloud only needs a profile name because a profile's sets are public`() {
        val preferences = SpicetifyPreferences(soundCloudUsername = "yabosen")
    ...
    fun `youtube music needs a cookie session`() {
        val preferences = SpicetifyPreferences(youtubeCookies = CookieSource.ofBrowser(BrowserSession.FIREFOX))
    ...
    fun `nothing configured means nothing to list`() {
        val preferences = SpicetifyPreferences()
...
class SoundCloudLibraryGateTest {
    ...
    fun `a soundcloud cookie session alone cannot locate playlists`() {
        val preferences = SpicetifyPreferences(soundCloudCookies = CookieSource.ofBrowser(BrowserSession.FIREFOX))
