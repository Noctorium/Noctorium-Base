...
class PlayerBarPositionTest {
    ...
    fun `the bar stays at the foot of the window unless asked otherwise`() {
        assertEquals(PlayerBarPosition.BOTTOM, SpicePreferences().playerBarPosition)
    }

    @Test
    fun `settings saved before the choice existed still open at the bottom`() {
        val older = """{"profileName":"Spice Listener","playerBarStyle":"INLINE"}"""

        val preferences = json.decodeFromString<SpicePreferences>(older)
    ...
    fun `the choice survives being written and read back`() {
        val chosen = SpicePreferences(playerBarPosition = PlayerBarPosition.TOP)

        val restored = json.decodeFromString<SpicePreferences>(json.encodeToString(chosen))
    ...
    fun `position and layout do not disturb each other`() {
        val stackedOnTop = SpicePreferences(
            playerBarStyle = PlayerBarStyle.STACKED,
            playerBarPosition = PlayerBarPosition.TOP,
        )

        val restored = json.decodeFromString<SpicePreferences>(json.encodeToString(stackedOnTop))
