...
class YtDlpPlaybackCookiesTest {
    private fun serviceWithBothSessions() = YtDlpService().apply {
        setCookieArguments(ProviderType.YOUTUBE_MUSIC, CookieSource.ofFile("C:/spice/youtube.cookies").ytDlpArguments())
        setCookieArguments(ProviderType.YOUTUBE_VIDEO, CookieSource.ofFile("C:/spice/youtube.cookies").ytDlpArguments())
        setCookieArguments(ProviderType.SOUNDCLOUD, CookieSource.ofFile("C:/spice/soundcloud.cookies").ytDlpArguments())
    }
    ...
    fun `soundcloud streams keep the session`() {
        ...
        assertEquals(
            listOf("--cookies", "C:/spice/soundcloud.cookies"),
    ...
    fun `browsing youtube still uses the session`() {
        ...
        assertEquals(
            listOf("--cookies", "C:/spice/youtube.cookies"),
