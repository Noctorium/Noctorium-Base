...
import app.spicetify.settings.AppDirectories
...
object BackendLocator {
    ...
    fun ytDlp(): Path? = locate("SPICETIFY_YTDLP_PATH", if (isWindows) "yt-dlp.exe" else "yt-dlp")
    fun mpv(): Path? = locate("SPICETIFY_MPV_PATH", if (isWindows) "mpv.exe" else "mpv")
    fun ffmpeg(): Path? = locate("SPICETIFY_FFMPEG_PATH", if (isWindows) "ffmpeg.exe" else "ffmpeg")
