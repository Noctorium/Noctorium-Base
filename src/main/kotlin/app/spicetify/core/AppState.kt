package app.spicetify.core

import app.spicetify.discord.DiscordPresenceManager
import app.spicetify.discord.DiscordPresenceSettings
import app.spicetify.discord.DiscordPresenceStatus
import app.spicetify.domain.*
import app.spicetify.lyrics.LyricsProviderId
import app.spicetify.lyrics.LyricsProviderOutcome
import app.spicetify.lyrics.LyricsProviderStatus
import app.spicetify.lyrics.LyricsRepository
import app.spicetify.lyrics.LyricsUiState
import app.spicetify.playback.QueueManager
import app.spicetify.playback.AccountProbe
import app.spicetify.playback.AccountProbeOutcome
import app.spicetify.playback.AccountProbeRequest
import app.spicetify.playback.BackendLocator
import app.spicetify.playback.MpvPlaybackEngine
import app.spicetify.playback.PlaybackEngine
import app.spicetify.playback.PlaybackState
import app.spicetify.playback.PlaybackStatus
import app.spicetify.playback.YtDlpService
import app.spicetify.playlists.LocalPlaylist
import app.spicetify.playlists.LocalPlaylistRepository
import app.spicetify.playlists.PlaylistShareLink
import app.spicetify.playlists.RecentTracksRepository
import app.spicetify.playlists.recentWith
import app.spicetify.playlists.shareableText
import app.spicetify.providers.MusicProvider
import app.spicetify.providers.YtDlpMusicProvider
import app.spicetify.scrobble.ScrobbleLog
import app.spicetify.scrobble.ScrobbleManager
import app.spicetify.social.LikeOutcome
import app.spicetify.social.SoundCloudLikeClient
import app.spicetify.social.SoundCloudToken
import app.spicetify.social.soundCloudCookieHeader
import app.spicetify.social.LikeResult
import app.spicetify.social.SoundCloudAccountClient
import app.spicetify.social.PlaylistWriteResult
import app.spicetify.social.SoundCloudClientIdProvider
import app.spicetify.social.SoundCloudPlaylistClient
import app.spicetify.social.InnertubeKeyProvider
import app.spicetify.social.YouTubeMusicClient
import app.spicetify.social.YouTubeChannel
import app.spicetify.social.YouTubeSession
import app.spicetify.social.cookieHeaderFor
import app.spicetify.account.AccountResult
import app.spicetify.account.ListeningStats
import app.spicetify.account.PlayReport
import app.spicetify.account.SpicetifyAccountClient
import app.spicetify.account.SpicetifyUser
 ...
 * Playlists made inside Spicetify are untouched, as are the other account's. `loaded` stays as it was so the next
...
data class LikeState(
    ...
    /** Channels the signed-in Google account owns, for choosing which one Spicetify acts as. */
...
class AppState(
    ...
    private val accountClient: SpicetifyAccountClient = SpicetifyAccountClient(),
) : AutoCloseable {
    ...
    private val mutableAccount = MutableStateFlow(SpicetifyAccountState())
    val account: StateFlow<SpicetifyAccountState> = mutableAccount.asStateFlow()
    /**
    ...
    fun setProfileName(name: String) {
        updatePreferences { copy(profileName = name.trim().take(40).ifBlank { "Spicetify Listener" }) }
    }
    ...
    /**
     * Likes or unlikes on the provider, not only inside Spicetify. The UI is moved first and rolled back if the
     ...
     * what lets Spicetify write likes at all. Only that one cookie is kept; any temporary jar is deleted.
     */
        ...
        scope.launch(Dispatchers.IO) {
            ...
            runCatching { credentials.put(SOUNDCLOUD_TOKEN, token) }
                .onSuccess {
                    ...
                    likeMessage("Liking is connected. Spicetify can now like tracks on your SoundCloud account.")
                    refreshLikes()
                }
    ...
    fun completeYouTubeSignIn(cookieFilePath: String) {
        ...
        updateAccountState(
            ...
            AccountConnectionState(AccountConnectionStatus.CONNECTED, "Signed in inside Spicetify."),
        )
        scope.launch {
    ...
    fun detectSoundCloudProfile(announce: Boolean = true) {
        scope.launch {
            val token = withContext(Dispatchers.IO) { runCatching { credentials.get(SOUNDCLOUD_TOKEN) }.getOrNull() }
            if (token.isNullOrBlank()) {
                if (announce) likeMessage("Sign in to SoundCloud first so Spicetify has a session to ask about.")
                return@launch
            }
    ...
    /** Directory Spicetify keeps its own files in, used for the embedded browser cache and exported session. */
    ...
    fun disconnectSoundCloudLiking() {
        ...
        likeMessage("Liking disconnected. Spicetify will not write to your SoundCloud account.")
    }
    ...
    private suspend fun readSoundCloudToken(source: CookieSource): String? {
        ...
        val jar = Files.createTempFile("spicetify-session", ".txt")
        return try {
    ...
    /** Switches which channel Spicetify acts as; every later call carries it. */
    ...
    /**
     * Makes a playlist on SoundCloud rather than only inside Spicetify. Private by default, since a playlist made
    ...
    /** Copies a playlist made in Spicetify up to YouTube Music, keeping only the tracks that live there. */
    ...
    /** Copies a playlist made in Spicetify up to SoundCloud, keeping only the tracks that live there. */
    ...
    // --- Playlists the listener builds inside Spicetify ---
    ...
    /** Copies the provider page for one track, which anyone can open with or without Spicetify. */
    ...
    /** Copies a link that carries the whole playlist, so another Spicetify can rebuild it without a server. */
    ...
    /** Copies a readable track listing for sharing with people who do not run Spicetify. */
    ...
    fun importSharedPlaylist(link: String) {
        ...
        if (shared == null) {
            libraryNotice("That does not look like a Spicetify playlist link.")
            return
        }
    ...
    /** Disconnecting means Spicetify forgets the account outright: session, profile name, like token, playlists. */
            ...
            runCatching { scrobbleManager.beginLastFmAuthorization() }
                .onSuccess { authorization ->
                    runCatching { withContext(Dispatchers.IO) { browseSecureUrl(authorization.url) } }
                        .onSuccess {
                            mutableSettings.update { it.copy(message = "Allow Spicetify in the Last.fm window that just opened.") }
    ...
    private fun updatePreferences(transform: SpicetifyPreferences.() -> SpicetifyPreferences) {
    ...
    private fun applyAccountPreferences(preferences: SpicetifyPreferences) {
    ...
    private fun describeSavedAccounts(preferences: SpicetifyPreferences) {
    ...
    private fun savedAccountState(source: CookieSource): AccountConnectionState = when {
        ...
        source.verifiedAtEpochSeconds != null -> AccountConnectionState(
            ...
            "Run Check connection so Spicetify can tell you whether this session really works.",
        )
    }
    ...
    // --- Spicetify account, and the listening it counts ---

    /** Picks a saved session back up, so signing in is something done once rather than every launch. */
    private fun restoreSpicetifyAccount() {
        scope.launch {
            val token = withContext(Dispatchers.IO) { runCatching { credentials.get(SPICETIFY_TOKEN) }.getOrNull() }
    ...
    fun signUpToSpicetify(email: String, password: String, displayName: String) =
        authenticateSpicetify { accountClient.signUp(email.trim(), password, displayName.trim()) }

    fun logInToSpicetify(email: String, password: String) =
        authenticateSpicetify { accountClient.logIn(email.trim(), password) }

    private fun authenticateSpicetify(attempt: suspend () -> AccountResult) {
        if (mutableAccount.value.busy) return
        scope.launch {
            mutableAccount.update { it.copy(busy = true, message = null) }
            when (val result = attempt()) {
                is AccountResult.Success -> {
                    withContext(Dispatchers.IO) { runCatching { credentials.put(SPICETIFY_TOKEN, result.token) } }
    ...
    fun signOutOfSpicetify() {
        scope.launch(Dispatchers.IO) { runCatching { credentials.remove(SPICETIFY_TOKEN) } }
        unreportedPlays.clear()
        mutableAccount.value = SpicetifyAccountState(message = "Signed out of your Spicetify account.")
    }

    fun clearSpicetifyMessage() = mutableAccount.update { it.copy(message = null) }
 ...
 */
internal fun SpicetifyPreferences.canListLibrary(provider: ProviderType): Boolean = when (provider) {
...
private fun SpicetifyPreferences.cookiesFor(slot: ProviderType): CookieSource =
    if (slot == ProviderType.SOUNDCLOUD) soundCloudCookies else youtubeCookies

private fun SpicetifyPreferences.withCookies(slot: ProviderType, source: CookieSource): SpicetifyPreferences =
...
/** Where Spicetify opens, chosen in Customization. */
...
/** Sign-in state for the Spicetify account, and the listening totals it keeps. */
data class SpicetifyAccountState(
    val user: SpicetifyUser? = null,
