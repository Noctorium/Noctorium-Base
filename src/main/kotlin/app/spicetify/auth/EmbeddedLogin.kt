...
/**
 * Hosts a provider's own sign-in page inside Spicetify using embedded Chromium.
 *
 * The listener types their password into the provider's real page, never into a Spicetify form, and the session
 * that results lives in this browser's own cookie store rather than in the system browser. That store is then
 * exported once as a cookie file for yt-dlp, and for the session-signed API calls Spicetify makes.
 *
 * Chromium ships inside Spicetify rather than being fetched on demand, so [start] only has to unpack it the first
...
fun writeCookieFile(cookies: List<HarvestedCookie>, destination: Path): Path {
    ...
    val lines = buildList {
        ...
        add("# Written by Spicetify from its embedded sign-in. Treat this file as a password.")
...
/** The page whose cookies Spicetify needs, which is the music site rather than the accounts one. */
