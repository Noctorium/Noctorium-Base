 ...
 *
 * Spice has no server, so a share link cannot point at hosted content. Instead the track list is compressed
 * into the link itself: anyone with Spice pastes it back and gets the same playlist, and it keeps working with
...
object PlaylistShareLink {
    const val PREFIX = "spice://playlist/1/"
    ...
    /** Returns null for anything that is not a readable Spice playlist link. */
...
/** A plain-text listing for sharing with people who do not have Spice. */
